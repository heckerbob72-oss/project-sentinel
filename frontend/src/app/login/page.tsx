...
export default function LoginPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await apiPost<LoginResponse>("/auth/login", parsed.data);
      const token = res.data.access_token;
      setAuth(token, { email: parsed.data.email });
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={onSubmit} className="panel space-y-4 p-6" noValidate>
          ...
          <p className="text-center text-xs text-muted-foreground">
            No account?{" "}
            <Link href="/register" className="text-sentinel-300 hover:underline">
              Request access
            </Link>
          </p>
        </form>
